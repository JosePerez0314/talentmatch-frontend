...
const Position = () => {
  ...
  if (isSuccess) return <PositionSuccess positionName={formData.role} onReset={() => { setIsSuccess(false); setFormData(INITIAL_STATE); setHasAttemptedSubmit(false); }} />;

  return (
    <div className="p-10 animate-fade-in max-w-5xl mx-auto">
      <form onSubmit={(e) => { e.preventDefault(); setHasAttemptedSubmit(true); if (validate()) setIsSuccess(true); }}
        className="bg-white rounded-[32px] p-12 shadow-sm border border-gray-100">

        <h1 className="text-2xl font-extrabold mb-10 text-center tracking-tight uppercase">Configuración de Posiciones</h1>

        <FormSection title="Detalles del rol">
          <FormField label="¿Cuál es el puesto que buscas?" error={hasAttemptedSubmit && errors.role}>
            <input type="text" placeholder="Ej. Senior Frontend" className={`p-4 bg-[#F8F9FA] border rounded-xl outline-none focus:border-[#447ECA] font-medium ${errors.role && hasAttemptedSubmit ? "border-red-500" : "border-[#D4D4DA]"}`}
              value={formData.role} onChange={e => setFormData({ ...formData, role: e.target.value })} />
          </FormField>

          <FormField label="Años de experiencia">
            <select className="w-full p-4 bg-[#F8F9FA] border border-[#D4D4DA] rounded-xl outline-none appearance-none font-medium cursor-pointer"
              value={formData.experience} onChange={e => setFormData({ ...formData, experience: e.target.value })}>
              <option value="0">Ninguno</option>
              {[...Array(10)].map((_, i) => <option key={i + 1} value={i + 1}>{i + 1} Año{i > 0 ? 's' : ''}</option>)}
        ...
        <FormSection title="Habilidades y cualificaciones">
          {[
            ...
            <PillInput key={c.id} {...c} pills={formData[c.id]} tempValue={tempInputs[c.id]} error={errors[c.id]} hasAttemptedSubmit={hasAttemptedSubmit}
              onAddPill={(id, v) => handlePill(id, v, 'add')} onRemovePill={(id, i) => handlePill(id, null, 'remove', i)}
          ...
          <FormField label="Describe el puesto solicitado" error={hasAttemptedSubmit && errors.description}>
            <textarea maxLength={3000} rows={4} className="p-4 bg-[#F8F9FA] border border-[#D4D4DA] rounded-xl outline-none focus:border-[#447ECA] resize-none font-medium"
        ...
        <FormSection title="Educación e idiomas">
          <FormField label="Educación necesaria">
            <input type="text" className="p-4 bg-[#F8F9FA] border border-[#D4D4DA] rounded-xl font-medium"
              value={formData.education} onChange={e => setFormData({ ...formData, education: e.target.value })} />
          </FormField>
          <PillInput label="Idiomas" id="languages" pills={formData.languages} tempValue={tempInputs.languages}
            onAddPill={(id, v) => handlePill(id, v, 'add')} onRemovePill={(id, i) => handlePill(id, null, 'remove', i)}
            onChangeTemp={(id, v) => setTempInputs({ ...tempInputs, [id]: v })} />
        </FormSection>

        <div className="flex justify-end pt-4">
          <button type="submit" className="bg-[#447ECA] text-white px-20 py-4 rounded-xl font-extrabold text-[13px] shadow-lg hover:bg-[#3669ab] active:scale-95 transition-all uppercase tracking-wider">
            Crear Posición
          </button>
        </div>
      </form>
