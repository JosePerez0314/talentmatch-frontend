...
const CandidateMatchRow = ({ resultData, index, onViewClick }) => {
    ...
    const { bg, text } = getMatchStyles(resultData.matchScore);
        ...
        <div className="grid grid-cols-[50px_100px_1fr_280px] gap-4 px-6 py-4 items-center border border-transparent border-b-gray-50 last:border-b-transparent hover:bg-white hover:border-[#447ECA]/30 hover:shadow-xl hover:shadow-blue-900/5 hover:rounded-[22px] transition-all duration-300 group cursor-pointer" onClick={onViewClick}>
            ...
            <div className="flex justify-center">
                <div className={`${bg} ${text} w-16 h-16 rounded-[22px] flex flex-col items-center justify-center shadow-lg shadow-gray-200 border border-black/10`}>
                    <span className="text-2xl font-black leading-none">{resultData.matchScore}%</span>
            ...
            <div className="flex flex-col pl-4">
                ...
                <span className="text-gray-400 text-xs mb-2">{person.email || "Sin email"}</span>
                <div className="inline-flex items-center gap-2 bg-gray-50 self-start px-2 py-1 rounded-lg border border-gray-100">
                    ...
                    <span className="text-gray-500 text-[10px] font-medium">{fileName}</span>
                </div>
            </div>
            ...
            <div className="flex items-center justify-end gap-4" onClick={(e) => e.stopPropagation()}>
                ...
                <button onClick={onViewClick} className="p-2.5 bg-white hover:bg-[#DCF9FF] rounded-xl transition-all border border-gray-100 hover:border-[#447ECA]/30" title="Ver análisis de IA">
                    <img src={Icons.stats.eyeHistoryGray} className="w-5 h-5 transition-opacity" style={{ filter: "invert(42%) sepia(85%) saturate(1212%) hue-rotate(189deg) brightness(91%) contrast(85%)" }} alt="ver" />
                </button>

                <a href={person.fileUrl} target="_blank" rel="noopener noreferrer" className="p-2.5 bg-white hover:bg-[#DCF9FF] rounded-xl transition-all border border-gray-100 hover:border-[#447ECA]/30 inline-block" title="Ver documento original">
                    <img src={Icons.stats.uploadCv} className="w-5 h-5 transition-opacity" style={{ filter: "invert(42%) sepia(85%) saturate(1212%) hue-rotate(189deg) brightness(91%) contrast(85%)" }} alt="doc" />
                </a>
            </div>
        </div>
    );
