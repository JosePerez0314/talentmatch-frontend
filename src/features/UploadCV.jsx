...
const UploadCV = () => {
    ...
    const validateFiles = (fileList) => {
        ...
        if (hasInvalid) {
            setError("Formato de archivo inválido. Por favor, carga de nuevo un archivo PDF compatible.");
            return [];
        }
    ...
    const FileItem = ({ file, index }) => (
        <div className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0 group">
            <div className="flex items-center gap-3">
                <div className="w-8 h-10 bg-gray-100 rounded flex items-center justify-center text-[10px] font-bold text-gray-400">PDF</div>
                <span className="text-gray-700 font-semibold text-sm truncate max-w-xs">{file.name}</span>
            </div>
            <div className="flex items-center gap-6">
                <span className="text-gray-300 text-xs font-medium">{(file.size / 1024).toFixed(0)} KB</span>
                <button
            ...
            <div className="bg-white rounded-[32px] shadow-sm p-8 border border-gray-100">
                ...
                <div
                    ...
                    {isDragging ? (
                        <div className="text-center pointer-events-none">
                            <h2 className="text-white text-6xl font-black mb-2 tracking-tighter italic">¡SÚELTALOS!</h2>
                            <p className="text-blue-400 font-bold uppercase text-xs tracking-[0.3em]">Arrastra y suelta el PDF aquí</p>
                        </div>
                    ) : (
                        <>
                            ...
                            <p className="text-gray-400 text-xs font-bold italic tracking-wide uppercase">O arrastra y suelta el PDF aquí</p>
                        </>
                    )}
                </div>
                ...
                {error && (
                    <div className="mt-8 p-5 bg-red-50 border-l-4 border-red-500 rounded-r-2xl flex items-center gap-4">
                        ...
                        <div>
                            <p className="text-red-600 font-black text-xs uppercase tracking-tight">Error de formato</p>
